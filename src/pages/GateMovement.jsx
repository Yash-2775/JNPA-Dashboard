...
export default function GateMovement() {
  ...
  const exportTrend = useMemo(
    () => currentList.map((m) => ({ month: m.monthName, value: m.expTeus || 0 })),
    [currentList]
  );

  const importTrend = useMemo(
    () => currentList.map((m) => ({ month: m.monthName, value: m.impTeus || 0 })),
    [currentList]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 flex justify-between">
        <h1 className="text-2xl font-semibold uppercase">Gate Movement</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Section title="Export" bg="bg-blue-100">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <KpiCard title="Current Month Export" value={latestMonth?.expTeus ?? "—"} />
            <KpiCard title="Prev. Month Export" value={previousMonth?.expTeus ?? "—"} />
          </div>
        ...
        <Section title="Import" bg="bg-orange-100">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <KpiCard title="Current Month Import" value={latestMonth?.impTeus ?? "—"} />
            <KpiCard title="Prev. Month Import" value={previousMonth?.impTeus ?? "—"} />
          </div>
          <ChartBox title="Import Trend | Monthly">
            <LineChart data={importTrend}>
              <XAxis dataKey="month" /><YAxis /><Tooltip />
              <Line dataKey="value" stroke="#ea580c" strokeWidth={2} />
            </LineChart>
          </ChartBox>
        </Section>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <h3 className="text-sm font-semibold mb-4">Gate Movement | Monthly</h3>
...
function ChartBox({ title, children }) {
  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h3 className="text-sm font-semibold mb-2 uppercase">{title}</h3>
